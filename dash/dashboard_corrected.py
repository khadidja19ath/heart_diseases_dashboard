...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with tabs[2]:
        ...
        st.markdown("#### 📍 Phase 1 — Lab 2 (Early Analysis): First impression was correct")
        st.markdown("""
        <div class="story-box success">
        <strong>✅ Initial finding (Lab 2):</strong><br><br>
        In our first analysis (using the notebook), <code>target = 1</code> means <strong>Infected / Diseased</strong>
        and <code>target = 0</code> means <strong>Healthy</strong>.<br><br>
        We found that <code>trestbps</code> was slightly higher in the <strong>healthy</strong> group.<br><br>
        According to medical guidelines (WHO):<br>
        • Normal BP &lt; 120 mmHg<br>
        • At risk: ~120–129 mmHg<br>
        • Heart disease likely: ≥ 130 mmHg<br><br>
        This appeared contradictory — healthy patients seemed to have higher BP. This puzzle is resolved in Phase 3.
        </div>""", unsafe_allow_html=True)

        # ── PHASE 2: EDA revealed the label flip ──
        st.markdown("#### 📍 Phase 2 — Lab 5 (EDA): The labels were flipped")
        ...
        st.markdown("""
        <div class="story-box warning">
        <strong>⚠️ The apparent contradiction (Lab 5):</strong><br><br>
        During EDA, we discovered the <strong>real label meaning</strong>:<br>
        • <code>target = 1</code> → <strong>Infected / Diseased</strong><br>
        • <code>target = 0</code> → <strong>Healthy</strong><br><br>
        Now the same data reads differently: <strong>healthy patients have higher trestbps than diseased ones</strong>.<br>
        This seems to contradict medical knowledge — heart disease is supposed to be associated with HIGH blood pressure.
        </div>""", unsafe_allow_html=True)
        ...
        • <code>target = 1</code> = Infected / Diseased (per notebook) · <code>target = 0</code> = Healthy<br>
        • Healthy patients show slightly higher mean <code>trestbps</code> — seems contradictory at first<br>
        ...
        • Lesson: <em>always interpret statistics within medical and contextual reality</em>
        </div>""", unsafe_allow_html=True)
