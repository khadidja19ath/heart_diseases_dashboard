...
if page == "📋  Overview":
    ...
    for col, val, lbl in [
        ...
        (c3, int(df['target'].sum()), "Diseased (1)"),
        (c4, int((df['target']==0).sum()), "Healthy (0)"),
    ]:
        col.markdown(f"""
    ...
    glossary = pd.DataFrame({
        ...
        'Meaning': ['Age in years','1=Male 0=Female','Chest pain type (0–3)','Resting blood pressure (mmHg)',
                    ...
                    'Thalassemia (0–3)','1 = Diseased · 0 = Healthy'],
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    # Gender analysis
    patients   = df[df['target'] == 1]   # infected/diseased (target=1 per notebook)
    ...
    with col2:
        ...
        fig.patch.set_facecolor('#1a1d2e')
        ax.set_facecolor('#1a1d2e')
        disease_rate = df.groupby('sex')['target'].apply(lambda x: (x==1).mean()*100)
    ...
    ⚠️ <strong>Observation:</strong> Males show a higher disease rate within their group compared to females.
    ...
    age_stats.index = ['Healthy (0)', 'Diseased (1)']
    ...
    with col2:
        ...
        fig.patch.set_facecolor('#1a1d2e')
        ax.set_facecolor('#1a1d2e')
        for tgt, col, lbl in [(0,'#3ecf8e','Healthy'), (1,'#e05c5c','Diseased')]:
    ...
    age_proba = df2.groupby('age_group', observed=False)['target'].apply(
        lambda x: (x==1).sum()/len(x)*100).reset_index()
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with col2:
        t_d = df[df['target']==1]['thalach'].mean()
        t_h = df[df['target']==0]['thalach'].mean()
        st.markdown(f"""
        <div class="story-box success">
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    g0 = df[df['target']==0]   # Healthy
    g1 = df[df['target']==1]   # Diseased
    t_rows = []
    for col in ['age','thalach','chol','trestbps']:
        t, p = stats.ttest_ind(g1[col], g0[col])
        t_rows.append({
            'Feature': col,
            'Mean (Diseased)': round(g1[col].mean(),1),
            'Mean (Healthy)':  round(g0[col].mean(),1),
            'Difference':      round(abs(g1[col].mean()-g0[col].mean()),1),
            'p-value':         round(p,4),
    ...
    ci_rows = []
    for tgt, name in [(1,'Diseased'), (0,'Healthy')]:
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with tabs[1]:
        ...
        for i, col in enumerate(num_cols):
            ...
            for tgt, clr, lbl in [(0,'#3ecf8e','Hlth'), (1,'#e05c5c','Dis.')]:
        ...
        for i, col in enumerate(cat_cols):
            axes[i].set_facecolor('#1a1d2e')
            for tgt, clr in [(0,'#3ecf8e'), (1,'#e05c5c')]:
                vals = df[df['target']==tgt][col].value_counts().sort_index()
                axes[i].bar([str(v) + ('h' if tgt==0 else 'd') for v in vals.index],
    ...
    with tabs[2]:
        st.markdown("### 🩸 The `trestbps` Journey — From Confusion to Clarity")

        # ── PHASE 1: Early analysis ──
        st.markdown("#### 📍 Phase 1 — Lab 2 (Early Analysis): Working with wrong labels")
        st.markdown("""
        <div class="story-box warning">
        <strong>⚠️ Initial assumption (Lab 2) — incorrect labels:</strong><br><br>
        In our earlier analyses, we were working under the <strong>wrong assumption</strong>:<br>
        • <code>target = 1</code> → assumed <strong>Healthy</strong><br>
        • <code>target = 0</code> → assumed <strong>Infected / Diseased</strong><br><br>
        Under that assumption, we found: <code>trestbps</code> was higher when <code>target = 0</code> (the group we thought was diseased).<br><br>
        According to medical guidelines (WHO):<br>
        • Normal BP &lt; 120 mmHg<br>
        • At risk: ~120–129 mmHg<br>
        • Heart disease likely: ≥ 130 mmHg<br><br>
        So the result <strong>seemed perfectly logical</strong> — diseased patients had higher BP ✅<br>
        We had no reason to question it at that stage.
        </div>""", unsafe_allow_html=True)

        # ── PHASE 2: EDA revealed the label flip ──
        st.markdown("#### 📍 Phase 2 — Lab 5 (EDA): Discovering the real label meaning")

        means = df.groupby('target')['trestbps'].mean()
        medians = df.groupby('target')['trestbps'].median()

        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Mean BP — Healthy (0)",  f"{means[0]:.1f} mmHg", "Higher ↑")
        c2.metric("Mean BP — Diseased (1)", f"{means[1]:.1f} mmHg")
        c3.metric("Median BP — Healthy (0)", f"{medians[0]:.0f} mmHg")
        c4.metric("Median BP — Diseased (1)", f"{medians[1]:.0f} mmHg")

        st.markdown("""
        <div class="story-box info">
        <strong>🔍 What EDA revealed — the label flip:</strong><br><br>
        During EDA, we went back to the dataset documentation and discovered the <strong>real label meaning</strong>:<br>
        • <code>target = 1</code> → <strong>Infected / Diseased</strong> ← opposite of what we assumed<br>
        • <code>target = 0</code> → <strong>Healthy</strong><br><br>
        Now re-reading the same data with the correct labels: <strong>healthy patients (0) have higher trestbps than diseased ones (1)</strong>.<br>
        This seems to contradict medical knowledge — heart disease is supposed to be associated with HIGH blood pressure.<br><br>
        So now we have a real question to answer: <em>why do healthy patients show higher resting BP?</em>
        </div>""", unsafe_allow_html=True)
        ...
        with col1:
            ...
            data_healthy  = df[df['target']==0]['trestbps']
            data_diseased = df[df['target']==1]['trestbps']
            bp = axes[0].boxplot([data_healthy, data_diseased],
                                  patch_artist=True,
                                  boxprops=dict(facecolor='none'),
                                  medianprops=dict(color='#f0a500', linewidth=2))
            bp['boxes'][0].set_facecolor('#3ecf8e33')
            bp['boxes'][1].set_facecolor('#e05c5c33')
            axes[0].set_xticklabels(['Healthy (0)', 'Diseased (1)'], color='#8b8fa8', fontsize=8)
            axes[0].set_title('BP by real label', color='#e8eaf2', fontsize=10)
            axes[0].tick_params(colors='#8b8fa8')
            for sp in axes[0].spines.values(): sp.set_visible(False)
            axes[1].set_facecolor('#1a1d2e')
            axes[1].hist(data_healthy,  bins=20, alpha=0.6, color='#3ecf8e', label='Healthy (0)',  edgecolor='none')
            axes[1].hist(data_diseased, bins=20, alpha=0.6, color='#e05c5c', label='Diseased (1)', edgecolor='none')
            axes[1].legend(facecolor='#1a1d2e', labelcolor='#e8eaf2', fontsize=8)
            axes[1].set_title('Distribution overlap', color='#e8eaf2', fontsize=10)
        ...
        with col2:
            st.markdown("""
            <div class="story-box">
            <strong>🔍 What the data actually shows:</strong><br><br>
            • Median is <strong>identical</strong> in both groups (~130 mmHg)<br>
            • Healthy group has <strong>higher std</strong> — driven by extreme outliers (up to 200 mmHg)<br>
            • The two distributions <strong>heavily overlap</strong><br>
            • The mean difference is <strong>only ~5 mmHg</strong> — very small<br><br>
            → <code>trestbps</code> has <strong>weak discriminative power</strong> in this dataset
            </div>""", unsafe_allow_html=True)

        # ── PHASE 3: Resolution ──
        st.markdown("#### 📍 Phase 3 — The Resolution: Not a contradiction, a medical reality")
        st.markdown("""
        <div class="story-box info">
        <strong>🏥 Why diseased patients can have LOWER resting blood pressure:</strong><br><br>
        1. 💊 Diseased patients are likely already on <strong>antihypertensive medication</strong> — artificially lowering their BP<br>
        2. 🏥 Cardiac patients are under <strong>closer medical supervision</strong> → better BP management<br>
        3. 📋 Many high-risk patients <strong>started treatment before</strong> entering this dataset<br>
        4. 🫀 In some cardiac conditions, the weakened heart produces <strong>lower resting pressure</strong> as a compensatory effect<br><br>
        <strong>This is NOT a data error. It is a real medical phenomenon.</strong>
        </div>""", unsafe_allow_html=True)

        st.markdown("""
        <div class="story-box success">
        <strong>📌 Final Conclusion on trestbps:</strong><br><br>
        • <strong>Lab 2 (wrong labels):</strong> target=0 assumed diseased → trestbps higher → seemed medically logical ✅<br>
        • <strong>Lab 5 (correct labels):</strong> target=1 is actually diseased → trestbps higher in healthy → apparent contradiction ⚠️<br>
        • <strong>Resolution:</strong> the contradiction is explained by medication, supervision, and cardiac compensation<br>
        • <strong>trestbps was NOT selected</strong> as a key feature — it lacks discriminative power alone<br>
        • Lesson: <em>always verify label definitions before interpreting results</em>
        </div>""", unsafe_allow_html=True)

        st.markdown("#### Stats comparison")
        col1, col2 = st.columns(2)
        with col1:
            st.markdown("**Healthy (target = 0)**")
            st.dataframe(df[df['target']==0]['trestbps'].describe().round(1).to_frame(),
                         use_container_width=True)
        with col2:
            st.markdown("**Diseased (target = 1)**")
    ...
    with tabs[3]:
        ...
        with col1:
            ...
            axes[0].set_facecolor('#1a1d2e')
            axes[0].bar(['Healthy\n(0)', 'Diseased\n(1)'], counts.values,
                        color=['#3ecf8e','#e05c5c'], edgecolor='none', width=0.5)
            ...
            axes[1].set_facecolor('#1a1d2e')
            axes[1].pie(counts.values, labels=['Healthy', 'Diseased'],
                        autopct='%1.1f%%', colors=['#3ecf8e','#e05c5c'],
        ...
        with col2:
            ...
            Healthy (0): <strong>{counts[0]} patients ({counts[0]/len(df)*100:.1f}%)</strong><br>
            Diseased (1): <strong>{counts[1]} patients ({counts[1]/len(df)*100:.1f}%)</strong><br><br>
