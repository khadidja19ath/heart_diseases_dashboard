...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with tabs[2]:
        ...
        In our first analysis, we noticed that <code>trestbps</code> was higher when <code>target = 0</code>.<br>
        At that stage, we assumed <strong>target = 0 meant Infected / Diseased</strong>.<br><br>
        According to medical guidelines (WHO):<br>
        • Normal BP &lt; 120 mmHg<br>
        • At risk: ~120–129 mmHg<br>
        • Heart disease likely: ≥ 130 mmHg<br><br>
        So our conclusion was: <strong>trestbps is higher in infected patients → medically correct ✅</strong><br>
        Everything made sense at this stage.
        </div>""", unsafe_allow_html=True)
        ...
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Mean BP — target=0",  f"{means[0]:.1f} mmHg", "Higher ↑")
        c2.metric("Mean BP — target=1",   f"{means[1]:.1f} mmHg")
        c3.metric("Median BP — target=0", f"{medians[0]:.0f} mmHg")
        c4.metric("Median BP — target=1",  f"{medians[1]:.0f} mmHg")
        ...
        • In <strong>Lab 2</strong> → target=0 seemed infected → trestbps higher → ✅ medically logical<br>
        • In <strong>Lab 5</strong> → target=1 is actually infected → trestbps still higher in healthy → seems contradictory<br>
